...
globals[
  ...
  current_number_of_turtles
  current_active_cases
  curr_lime_population
...
to setup_world
  reset-ticks
  clear-turtles
  set student_id 22074971
  set student_name "Mohammed kaif"
  ...
  ask patches with [pycor <=  0][set pcolor lime]
...
to setup_agents
  ...
  ask n-of initially_infected turtles with
  [
    group = "lime turtle"]
  [
    setxy 0 min-pycor - 20
    infection
  ]
  ask n-of initially_infected turtles with
  [
...
to recurrview
  set curr_lime_population
  count turtles with [ group = "lime turtle" ]
  set present_cyan_population
  count turtles with [ group = "cyan turtle" ]
  set current_number_of_turtles count turtles
  if   current_number_of_turtles > 0
  [
  set total_infected_percentage 100 *(count turtles with [ color = red ])/ current_number_of_turtles
  ]
  if curr_lime_population > 0
  [
    set lime_infected_percentage 100 *(count turtles with [color = red and group = "lime turtle"])/ curr_lime_population
  ]
  if present_cyan_population > 0
  [
    set cyan_infected_percentage (count turtles with [color = red and group = "cyan turtle"])* 100 / present_cyan_population
  ]
  if current_number_of_turtles > 0
  [
    set total_antibodies_percentage (count turtles with [ color = black])* 100 /  current_number_of_turtles
  ]
  if curr_lime_population > 0
  [
    set lime_antibodies_percentage (count turtles with [color = black and group = "lime turtle"]) * 100 / curr_lime_population
  ]
  if present_cyan_population > 0
  [
    set cyan_antibodies_percentage (count turtles with [color = black and group = "cyan turtle"]) * 100 / present_cyan_population
  ]
  set current_active_cases count turtles with [alter]
end
to run_model
  ask turtles
  [
    iteration
    turtle_motion
    if color = red or color = orange [ die_leave ]
    if alter [ inject ]
  ]
  recurrview
  reset-ticks
...
to restrict_travelling
  if group = "lime turtle" [
    ( ifelse ycor >= 0 [ set heading -90 forward 0.2 ]
...
to turtle_motion
  right one-of (range -20 21)
  ( ifelse social_distancing [social_Movement]
    self_isolation [ do_isolate ]
    travel_restrictions [ restrict_travelling ]
    [ fd 0.2 ] )
end
to immune_turtle
